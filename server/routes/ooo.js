    ...
    if (referer || process.env.NODE_ENV == 'DEVELOPMENT') {
        ...
        db.OOOuser.findAll({
            ...
            if (SQLResult.length == 0) {
                ...
                formatedStartTime = dateFormat(Date.now(), [format = 'DD-MM-YYYY - HH:mm'])
                formatedEndTime = ''
                Message = ''
                summaryRequired = true
            } else {

                formatedStartTime = dateFormat(SQLResult[0].StartTime, [format = 'DD-MM-YYYY - HH:mm'])
                if (SQLResult[0].EndTime) {
                    formatedEndTime = dateFormat(SQLResult[0].EndTime, [format = 'DD-MM-YYYY - HH:mm'])
                } else {
